<?php
if(!class_exists("EnteraddonsBase")) {
	class EnteraddonsBase {
    	public $key = "2F27690E25A9A274";
    	private $product_id = "66";
    	private $product_base = "enteraddons-pro";
		...
		function __construct($plugin_base_file='')
		{
			...
			if($this->hasCheckUpdate) {
				if(function_exists("add_action")){
					add_action( 'admin_post_enteraddons_fupc', function(){
					...
					if($this->isTheme){
						...
						add_filter( 'plugin_row_meta', function($links, $plugin_file ){
							if ( $plugin_file == plugin_basename( $this->pluginFile ) ) {
								$links[] = " <a class='edit coption' href='" . esc_url( admin_url( 'admin-post.php' ) . '?action=enteraddons_fupc' ) . "'>Update Check</a>";
							}
							return $links;
						}, 10, 2 );
		...
		function handleServerRequest(){
			...
			switch ($type) {
				case "rl": //remove license
					...
					$obj->status  = true;
					echo apply_filters( 'enteraddons_rl_encrypt_obj', $this->encryptObj( $obj ));
				...
				case "rc": //remove license
					...
					$obj->product = $this->product_id;
					$obj->status  = true;
					echo apply_filters( 'enteraddons_rc_encrypt_obj', $this->encryptObj( $obj ));
					return;
				case "dl": //delete plugins
					...
					if ( $this->isTheme ) {
						...
						echo apply_filters( 'enteraddons_dl_encrypt_obj', $this->encryptObj( $obj ));
					} else {
