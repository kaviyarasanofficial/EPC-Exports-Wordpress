...
class Loader {
	...
	public function __construct() {
		...
		add_action( 'admin_init', [ $this, 'detect_saas_env' ] );
	...
	public function define_constants() {
		...
		$saas_env = OptionController::get_option( 'saas_env' );
		if ( ! empty( $saas_env ) ) {
			$sass_url    = "https://{$saas_env}.suretriggers.com";
			$api_url     = "https://api-{$saas_env}.suretriggers.com";
			$webhook_url = "https://webhook-{$saas_env}.suretriggers.com";
		}
		...
		define( 'SURE_TRIGGERS_VER', '1.0.23' );
		define( 'SURE_TRIGGERS_DB_VER', '1.0.23' );
	...
	/**
	 * Switch saas env from URL
	 *
	 * @return void 
	 */
	public function detect_saas_env() {
		$saas_env = [ 'app', 'stage', 'dev', 'qaing', 'release' ];

		$env = sanitize_text_field( wp_unslash( isset( $_GET['env'] ) ? $_GET['env'] : '' ) ); // phpcs:ignore WordPress.Security.NonceVerification.Recommended

		if ( ! current_user_can( 'manage_options' ) || ( empty( $env ) || ! in_array( $env, $saas_env, true ) ) ) {
			return;
		}

		OptionController::set_option( 'saas_env', $env );
		wp_safe_redirect( admin_url( 'admin.php?page=suretriggers' ) );
		exit();
	}
}
